        ...
        public void SendInterviewInvitation(InterviewEmailData data)
        {
            try
            {
                ...
                using (var client = new SmtpClient())
                {
                    ...
                    client.Connect(ConfigurationManager.AppSettings["SmtpServer"], int.Parse(ConfigurationManager.AppSettings["Port"]), SecureSocketOptions.StartTls);
                    client.Authenticate(ConfigurationManager.AppSettings["SenderEmail"], ConfigurationManager.AppSettings["Password"]);
                    client.Send(message);
